...
public interface ProjectRepository extends JpaRepository<Project, Long>, JpaSpecificationExecutor<Project> {
    ...
    @Query("SELECT COUNT(p) FROM Project p WHERE p.status = 'OPEN_FOR_VOTING'")
    long countActiveVotations();


    @Query("SELECT p FROM Project p WHERE p.status = 'OPEN_FOR_VOTING' ORDER BY p.votingEndAt ASC")
    ...
    @Query("""
        SELECT p FROM Project p 
        LEFT JOIN FETCH p.creator
        WHERE p.status = 'OPEN_FOR_VOTING'
    ...
    @Query("""
        SELECT p FROM Project p
        WHERE p.votingStartAt = :today
        AND p.status = 'READY_TO_PUBLISH'
    """)
    List<Project> findProjectsToOpenVoting(@Param("today") LocalDate today);
}